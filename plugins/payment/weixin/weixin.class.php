...
class weixin extends RelationModel
{
    ...
    function getJSAPI($order)
    {
        ...
        if ($order['attach'] == 'db') {
            $go_url = U('Mobile/OneShop/pay_success', array('order_id' => $order['order_id']));
            $back_url = U('Mobile/OneShop/back_order', array('order_id' => $order['order_id']));
            $notify_url = SITE_URL . '/index.php/Mobile/OneShop/notifyUrl/order_id/' . $order['order_id'];
        }
