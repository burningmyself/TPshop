 ...
 * @callback string  回调函数(单张图片返回保存路径字符串，多张则为路径数组 )
 */
function UploadifyVideo(num,elementid,path,callback)
{
    var upurl ='/index.php?m=Admin&c=Uploadify&a=UploadifyVideo&num='+num+'&input='+elementid+'&path='+path+'&func='+callback;
    var iframe_str='<iframe frameborder="0" ';
    iframe_str=iframe_str+'id=uploadify ';
    iframe_str=iframe_str+' src='+upurl;
    iframe_str=iframe_str+' allowtransparency="true" class="uploadframe" scrolling="no"> ';
 ...
 * 删除视频input
 * @access   public
 * @val  string  删除视频input
 */
function ClearVideo(val)
{
    $("li[rel='"+ val +"']").remove();
    $.get(
